...
df_state = df_state %>%
  ...
  mutate(d_inst = c(NA, diff(inst)),
    ...
    d_ag_vis7_ined2 = c(NA, diff(ag_vis7_ined2)),
    d_ag_lb_ined = c(NA, diff(ag_lb_ined))
  ) %>%
  # Replace rest_inst for the year 2002
  mutate(rest_inst = ifelse(year == 2002, (l_inst - d_inst), rest_inst))
var_list0 = c("util_rate", "right", "ag_ltotal", "ag_ssaude", "ag_lsaude", "ag_vis7_ined2", "ag_lb_ined")
var_list1 = c("d_util_rate", "d_right", "d_ag_ltotal", "d_ag_ssaude", "d_ag_lsaude", "d_ag_vis7_ined2", "d_ag_lb_ined")
...
mean_sd_list$ag_lb_ined = (mean_sd_list$ag_lb_ined)*100
...
model1 = lm(as.formula(paste0(var_list2[1], "~", paste0(covars[3:25], collapse = " + "))), data = data_filtered)
  
data_filtered$z = residuals(model1)
  
for (i in 1:length(var_list1[1:5])) {
  var = var_list1[i]
  model2 = lm(as.formula(paste(var, "~", paste(covars[3:25], collapse = " + "))), 
           data = data_filtered)
...
# Fit regression model with the specified dependent variable and predictors
model1 = lm(as.formula(paste0(var_list2[2], "~", paste0(covars[3:25], collapse = " + "))), data = data_filtered)
...
# Fit regression model with the specified dependent variable and predictors
model1 = lm(as.formula(paste0(var_list2[3], "~", paste0(covars[3:52], collapse = " + "))), data = df_state)
...
# Fit regression model with the specified dependent variable and predictors
model1 = lm(as.formula(paste0(var_list2[4], "~", paste0(covars[3:52], collapse = " + "))), data = df_state)
...
output_table[c(7, 13), 2] = c("—", "—")
output_table[c(8, 14), 2] = c("", "")
...
df_state = df_state %>%
  # Generate d_inst and l_inst
  mutate(d_ag_munic_ssaude = c(NA, diff(ag_munic_ssaude)),
    d_ag_gini = c(NA, diff(ag_gini)),
    d_ag_lpop = c(NA, diff(ag_lpop)),
    d_ag_lgdp = c(NA, diff(ag_lgdp)),
    d_ag_poverty = c(NA, diff(ag_poverty)),
    d_ag_vis7_educ2 = c(NA, diff(ag_vis7_educ2)),
    d_ag_lb_educ = c(NA, diff(ag_lb_educ))
  )
var_list0 = c("ag_munic_ssaude", "ag_lpop", "ag_lgdp", "ag_poverty", "ag_gini", "ag_vis7_educ2", "ag_lb_educ")
var_list1 = c("d_ag_munic_ssaude", "d_ag_lpop", "d_ag_lgdp", "d_ag_poverty", "d_ag_gini", "d_ag_vis7_educ2", "d_ag_lb_educ")
var_list2 = c("d_inst98e", "d_inst02e", "inst", "rest_inst")
# Calculate means and standard deviations for each variable
coef_list = list()
se_list = list()
...
mean_sd_list$ag_lb_educ = (mean_sd_list$ag_lb_educ)*100

covars = c("ag_vis_educ_cover", "ag_lb_educ_cover", colnames(df_state %>% 
  select(starts_with("reg") | starts_with("delec") | starts_with("duf"))))
# Filter data for the specified year
data_filtered = df_state %>%
  filter(year == 1998)
...
for (j in 1:length(var_list0)) {
  var = var_list0[j]
  output_table[3*j - 2, 2] = sprintf("%.3f", coef_list[[paste0(var, j)]])
  output_table[3*j - 1, 2] = sprintf("(%.3f)", se_list[[paste0(var, j)]])
  output_table[3*j, 2] = sprintf("(%.3f)", p_value_list[[paste0(var, j)]])
}
for (j in 1:length(var_list1)) {
  var = var_list1[j]
  var2 = var_list0[j]
