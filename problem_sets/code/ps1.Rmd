...
# Estimating beta using GMM
beta_1 = (p[1] * c[, 1] ^ (-alpha)) / (r[2] * c[, 2] ^ (-alpha))
beta_hat_1 = mean(beta_1)

beta_2 = (p[2] * c[, 2] ^ (-alpha)) / (r[3] * c[, 3] ^ (-alpha))
beta_hat_2 = mean(beta_2)
...
We can then write the Jacobian matrix $\frac{\partial J(\beta)}{\partial \beta}$ in the following way
$$ \frac{\partial J(\beta)}{\partial \beta} = \binom{2 a_{11} \beta_1+\left(a_{12}+a_{21}\right) \beta_2}{\left(a_{12}+a_{21}\right) \beta_1+2 a_{22} \beta_2} $$
But we have that $2A\beta$ is equal to $$
2 A \beta=2\left(\begin{array}{ll}
a_{11} & a_{12} \\
a_{21} & a_{22}
\end{array}\right)\binom{\beta_1}{\beta_2}=\binom{2\left(a_{11} \beta_1+a_{12} \beta_2\right)}{2\left(a_{21} \beta_1+a_{22} \beta_2\right)}
$$

Therefore, $\frac{\partial J(\beta)}{\partial \beta} = 2A\beta$ if and only if $a_{ij}=a_{ji}, \forall i \neq j$, i.e., if and only if A is symmetric.
...
0, & \text{otherwise }.
\end{cases} $$
Now, we know that the kernel density estimator is given by: $$ \hat{f}\left(x_0\right)=\frac{1}{n h} \sum_{i=1}^n K\left(\frac{x_i-x_0}{h}\right) $$

Since we are using a triangular kernel, $h=2$ and given $z = \frac{x_i-x_0}{h}$. It is possible to write the kernel density estimator the following way: $$ \hat{f}\left(x_0\right)=\frac{1}{2n} \sum_{i=1}^n (1-|z|) I(|z|<1) $$ 

Assuming that the sample is i.i.d., we obtain
